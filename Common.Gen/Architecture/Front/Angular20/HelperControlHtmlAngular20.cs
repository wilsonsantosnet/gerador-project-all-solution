...
namespace Common.Gen
{
    ...
    public class HelperControlHtmlAngular20 : HelperControlHtml
    {

        #region Form Fields

        public override string MakeInputHtml(Context configContext, TableInfo tableInfo, Info info, bool isStringLengthBig, string propertyName)
        {
        ...
        public override string MakeCtrl(string ctrl)
        {
      ...
      " + ctrl + @"
        ...
        public override string MakeCheckbox(bool isRequired, string attr)
        {
      ...
      <label>
        ...
        public override string MakeRadio(bool isRequired, string attr)
        {
            var required = string.Empty;
...
<div class='form-group'>
    ...
    <label class='radio-inline'><input type='radio' [(ngModel)]='vm.model.<#propertyName#>' " + required + @" name='<#propertyName#>' formControlName='<#propertyName#>' " + attr + @" value='false' [checked]='!vm.model.<#propertyName#>' (change)='vm.model.<#propertyName#>=false'> Não</label>
      ...
      <label class='mr-1'>{{ vm.infos | traduction:'<#propertyName#>' }}</label> <#componentNameBtn#>
        ...
        public override string MakeDropDownSeach(bool isRequired, string attr)
        {
        ...
        public override string MakeTextEditor(bool isRequired)
        {
            ...
            var init = "[init] = \"{ plugins: 'lists link image table code help wordcount' }\"";
            var apiKey = "apiKey=w0yc6uopropgykx3pj1g5ehupy206vtq08u10xfnnl15gwno";
            return @"    <div class='form-group'>
              <label>{{ vm.infos | traduction:'<#propertyName#>' }}</label>
              <editor [(ngModel)]='vm.model.<#propertyName#>' name='<#propertyName#>' " + required + @" formControlName='<#propertyName#>'" + init + " " + apiKey + @">
              </editor>
            </div>";
        }

        public override string MakeTextTag(bool isRequired)
        {
            var required = isRequired ? "required='true'" : string.Empty;
            return @"    <div class='form-group'>
      <label>{{ vm.infos | traduction:'<#propertyName#>' }}</label>
      <tag-custom [ngModel] =" + "\"vm.model.<#propertyName#>\" (tagChange)=\"vm.model.<#propertyName#>=$event\" formControlName='<#propertyName#>' " + required + @"></tag-custom>
        ...
        public string MakeTextEditorBasic(bool isRequired)
        {
            var required = isRequired ? "required='true'" : string.Empty;
            return MakeTextArea(required, "(editorKeyup)='onEditorKeyup<#propertyName#>($event)'");
        }

        public override string MakeTextStyle(bool isRequired)
        {
            var required = isRequired ? "required='true'" : string.Empty;
      ...
      <fieldset>
        ...
        public override string MakeUpload(Info info)
        {
        ...
        public override string MakeInputFilterHtml(string attr = "")
        {
      ...
      " + ctrl + @"
        ...
        public override string MakeDropDownFilter(string attr = "")
        {
            return @"<div class='form-group'>
      <label>{{ vm.infos | traduction:'<#propertyName#>'}}</label>
      <select class='form-control'  name='<#propertyName#>' [(ngModel)]='vm.modelFilter.<#propertyName#>' datasource [dataitem]=" + "\"'<#ReletedClass#>'\"" + " [fieldFilterName]=\"'<#fieldFilterName#>'\"" + attr + @"></select>
        ...
        public override string MakeDatePikerFilter(string propertyName = "<#propertyName#>")
        {
      ...
      <label>
        ...
        public override string MakeRadioFilter()
        {
            return @"<label>{{ vm.infos | traduction:'<#propertyName#>' }}</label>
<div class='form-group'>
    ...
    <label class='radio-inline'><input type='radio' name='<#propertyName#>' [(ngModel)]='vm.modelFilter.<#propertyName#>' value='false'> Não</label>
        ...
        public override string MakeMultiSelectFilter()
        {
            return @"<div class='form-group'>
        <label>{{ vm.infos | traduction:'<#propertyName#>' }}</label>
        ...
        public override string MakeDropDownSeachFilter()
        {
        ...
        #region Helpers
        private string DefineAttr(Context configContext, TableInfo tableInfo, Info info, string propertyName)
        {
        ...
        protected string MakeInputType(string required, string type, string attr = "")
        {
            return @"    <div class='form-group'>
      <label>{{ vm.infos | traduction:'<#propertyName#>' }}</label>
      <input type='" + type + @"' class='form-control' [(ngModel)]='vm.model.<#propertyName#>' name='<#propertyName#>' " + required + @" formControlName='<#propertyName#>' " + attr + @" autocomplete='off'/>
        ...
        private string MakeCPFCNPJTab(string required, string attr)
        {
            return @"    <div class='form-group'>
      <label>{{ vm.infos | traduction:'<#propertyName#>' }}</label>
      <tabset>
        ...
        private string MakeCEP(bool required, string attr)
        {
            return @"<div class='form-group'>
                      <label>{{ vm.infos | traduction:'<#propertyName#>' }}</label>
        ...
        private string DefineMask(string dataType)
        {
        ...
        private string MakeInputPasswordConfirmation(string required, string type)
        {
      ...
      <input type='" + type + @"' class='form-control'  [(ngModel)]='vm.model.confSenha' name='confSenha' " + required + @" formControlName='confSenha' />
        ...
        private string MakeTextArea(string required, string attr = null)
        {
            return @"    <div class='form-group'>
      <label>{{ vm.infos | traduction:'<#propertyName#>' }}</label>
